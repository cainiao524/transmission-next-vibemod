  ...
  return (
    <Card className="overflow-hidden border-none bg-sidebar/30 py-0 shadow-md">
      ...
      <CardContent className="relative h-36 p-4 sm:h-52">
        ...
        {history.length < 2 && <div className="absolute inset-0 flex items-center justify-center text-sm text-muted-foreground">{t("speed_chart.waiting")}</div>}
        <span className="absolute bottom-1 right-4 text-[10px] text-muted-foreground/50">{t("speed_chart.samples", { count: history.length })}</span>
      </CardContent>
    </Card>
  )
}
