...
export function buildTorrentFileTree(
  files: TorrentFile[],
): TorrentFileTreeNode[] {
...
export function getTorrentFileSearchKeys(
  ...
  while (stack.length) {
    ...
    for (let index = 1; index <= segments.length; index++)
      result.add(`folder:${segments.slice(0, index).join("/")}`);
  }
  return result;
}
...
export function flattenVisibleTorrentFileTree(
  ...
  const value = (node: TorrentFileTreeNode): string | number => {
    ...
    if (sort.key === "progress")
      return node.length > 0 ? node.bytesCompleted / node.length : 0;
  ...
  const isDefaultOrder = sort.key === "name" && sort.direction === "asc";
  const ordered = (items: TorrentFileTreeNode[]) =>
    isDefaultOrder ? items : [...items].sort(compare);
  const result: VisibleTorrentFileTreeNode[] = [];
  const stack = ordered(nodes)
  ...
  while (stack.length) {
    ...
    result.push(current);
    const shouldExpand =
      current.node.kind === "folder" && expanded.has(current.node.key);
...
/**
 * 在默认排序（名称升序）且无搜索时，展开文件夹只需把其直接子节点
 * 插入到可见列表的对应位置，无需全量重建。
 */
export function expandVisibleNode(
  nodes: VisibleTorrentFileTreeNode[],
  node: TorrentFileTreeNode,
  key: string,
): VisibleTorrentFileTreeNode[] {
  if (node.kind !== "folder") return nodes;
  const idx = nodes.findIndex((entry) => entry.node.key === key);
...
/** 折叠文件夹时移除其所有已展开的后代节点。 */
export function collapseVisibleNode(
  nodes: VisibleTorrentFileTreeNode[],
  key: string,
): VisibleTorrentFileTreeNode[] {
  const idx = nodes.findIndex((entry) => entry.node.key === key);
