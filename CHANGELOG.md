...
- 修复浏览器高缩放或窄桌面宽度下，种子详情文件列表撑宽整个页面并覆盖侧栏的问题
- 文件表头与数据行改为在列表容器内横向滚动，并保持双向滚动同步与列对齐
- 保留正常桌面宽度、手机端布局、大量文件虚拟列表及排序后的页面位置

### 构建 / 发布

- README、DOCKERHUB、应用版本与镜像标签同步至 v1.17.3-baka9

## v1.17.2-baka9

### 文件列表

- 修正大量文件详情页在列表中间点击排序后短暂显示空白的问题
- 排序名称、大小、进度或优先级时保留当前页面滚动位置，不再重置虚拟列表偏移
- 增加大量文件中段排序的虚拟列表位置回归测试

### 构建 / 发布

- README、DOCKERHUB、应用版本与镜像标签同步至 v1.17.2-baka9
